...
const FormPegawai = ({ data, mode }: { data?: any; mode?: string }) => {
    ...
    const [image, setImage] = useState(data ? `/api/image/users/${data.user_image}` : '/users/nopic.png');
    ...
    const processForm: SubmitHandler<Inputs> = async (dt) => {
        ...
        const result: any = data ? await updateUser(formData, data.user_id) : await saveUser(formData);
        if (!result) {
    ...
    return (
        <form onSubmit={handleSubmit(processForm, onFormError)} className="flex flex-col gap-2.5 xl:flex-row">
            <div className="panel flex-1 px-0 py-6 ltr:xl:mr-6 rtl:xl:ml-6">
                <div className="flex flex-wrap justify-between px-4">
                    <div className="mb-6 w-full lg:w-1/2">
                        <div className="flex shrink-0 items-center text-black dark:text-white">
                            <Image src={image} alt="img" className="w-32" width={300} height={300} />
                        </div>
                    ...
                    <div className="w-full lg:w-1/2 lg:max-w-fit">
                        <div className={clsx('flex items-center', { 'has-error': errors.user_nip })}>
                ...
                <div className="mt-8 px-4">
                    <div className="flex flex-col justify-between lg:flex-row">
                        <div className="mb-6 w-full lg:w-1/2 ltr:lg:mr-6 rtl:lg:ml-6">
                            ...
                            <div className={clsx('mt-4 flex items-center', { 'has-error': errors.user_nama })}>
                                <label htmlFor="reciever-name1" className="mb-0 w-1/3 ltr:mr-2 rtl:ml-2">
                                    Nama Lengkap
                                </label>
                                <input
                            ...
                            <div className={clsx('mt-4 flex items-center', { 'has-error': errors.user_email })}>
                                <label htmlFor="reciever-email1" className="mb-0 w-1/3 ltr:mr-2 rtl:ml-2">
                                    Email
                                </label>
                                <input id="reciever-email1" type="email" {...register('user_email')} className="form-input flex-1" placeholder="Enter Email" />
                            </div>
                            <div className={clsx('mt-4 flex items-center', { 'has-error': errors.user_alamat })}>
                                <label htmlFor="reciever-address1" className="mb-0 w-1/3 ltr:mr-2 rtl:ml-2">
                                    Alamat
                                </label>
                                <input
                ...
                <div className="mt-8 px-4">
                    <div className="flex flex-col justify-between lg:flex-row">
                        <div className="mb-6 w-full lg:w-1/2 ltr:lg:mr-6 rtl:lg:ml-6">
                            ...
                            <div className={clsx('mt-4 flex items-center', { 'has-error': errors.user_name })}>
                                <label htmlFor="reciever-name" className="mb-0 w-1/3 ltr:mr-2 rtl:ml-2">
                                    Username
                                </label>
                                <input id="reciever-name" type="text" {...register('user_name')} className="form-input flex-1" placeholder="Enter Name" />
                            </div>
                            <div className={clsx('mt-4 flex items-center', { 'has-error': errors.user_pass })}>
                        ...
                        <div className="w-full lg:w-1/2">
                            ...
                            <div className={clsx('mt-4 flex items-center', { 'has-error': errors.confirm_password })}>
                                ...
                                <input id="confirm_password" type="password" {...register('confirm_password')} className="form-input flex-1" placeholder="Enter confirm password" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="panel">
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-1">
                        <button type="submit" className="btn btn-success w-full gap-2" disabled={typeof loading === 'string'}>
                            {loading ? (
                                <>
                                    <span className="inline-block h-3 w-3 animate-ping rounded-full bg-white ltr:mr-4 rtl:ml-4"></span> Loading ...
                                </>
                            ) : (
                                <>
                        ...
                        <Link href={'/pegawai'} className="btn btn-dark w-full gap-2">
